...
const updateApplication = async (req, res) => {
  try {
    ...
    await Promise.all(documentsURL.map(async (doc) => {
      const existingDocumentIndex = application.documents.findIndex(existingDoc => existingDoc.document === doc.document);
...
const completeApplication = async (req, res) => {
  try {
    ...
    const { appId } = req.params;
    const application = await Application.findByIdAndUpdate(appId, { completedHash:hash, completedDocURL:url });
    console.log(application)
...
module.exports = { createApplication, updateApplication, completeApplication, getApplication, getMyApplications, getAllApplications, getApplicationStatistics, approveApplication, rejectApplication, uploadDocumentToApplication, getDocumentsForApplication, updateApplicationStatus }