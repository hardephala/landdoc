        ...
        <div className="row justify-content-center">
          ...
          <div className="col-md-6">
            ...
            { selectedApplication?.completedDocURL && (
              <p>
                {`Completed ${application?.appType?.applicationName} Document`} :{' 1'}
          ...
          <p><strong>Document Files:</strong></p>
            <ul>
              {selectedApplication?.appType.requiredDocuments?.map((file, index) => (
                <div key={file._id}>
                    <div className="col-md-6">
                        <span style={{marginRight:"10px"}}><strong>Document:</strong> {file.document}</span>
                        <span style={{ marginRight: "10px" }}>
                          <strong>Status:</strong> {selectedApplication.documents[index]?.status || "Action Needed"}
                        </span>
                        {' '}
                        {selectedApplication.documents[index] && (
                          <a href={`http://localhost:4000/${selectedApplication.documents[index]?.url}`} target="_blank" rel="noopener noreferrer">
                            View in new tab
                          </a>
                        )}
                    </div>
